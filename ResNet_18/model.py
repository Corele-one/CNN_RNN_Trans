...
class Residual(nn.Module):
    def __init__(self ,input_channels ,num_channels ,use_1conv = False,strides = 1): #strides默认为1，在网络中strides一般会为2
        ...
        self.bn2 = nn.BatchNorm2d(num_channels) #在网络中，有了BN层的话一般就不用初始化权重了（w和b）
        if use_1conv:
            self.conv3 = nn.Conv2d(in_channels=input_channels, out_channels=num_channels, kernel_size=1, padding=0,stride=strides)
        else:
            self.conv3 = None
